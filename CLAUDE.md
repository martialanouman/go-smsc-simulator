...
- **JAMAIS d'horloge murale sur un chemin déterministe.** En mode graîné (`seed` défini), tout mécanisme temporel **planifié** (DLR, MO, `spike`, déconnexions, transitions) lit `per_bind_clock`. Jamais `time.Now()`, jamais un PRNG non graîné sur le chemin de décision. Le mode chaos (`seed` absent) est le **seul** endroit où l'horloge murale est permise. **Seule exception** : le plafond de débit (`throughput_cap_per_sec`/`throughput_limit_per_sec`) est un mécanisme **réactif temps réel** (fenêtre d'1 s sur l'horloge murale), et non planifié — un débit « par seconde » n'a pas de sens sur un compteur de PDU. Les profils `throttling-carrier`/`throughput-capped` sont donc **hors du corpus de rejeu** de l'invariant (a) : garantie d'agrégation statistique par bind seulement (spec §6.2/§6.3), assertés par seuil. L'invariant (a) se prouve sur `flaky-carrier`.
